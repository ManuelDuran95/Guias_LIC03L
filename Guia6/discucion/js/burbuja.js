function inicializar() {
    ...
    var i, max, temp, contenido = "", tdelement,aux1;
    ...
    do{
        aux1=prompt("Seleccione el orden: 1-Asendente 2-Desendente","");
        if(isNaN(aux1)){
            alert("seleccione 1 para asendente o 2 para desendente");
            temp=0;
            continue;
        }
        if(aux1==null){
    ...
    with (document) {
        ...
        //Seleccionado el metodo que el usuario ha elejido
        if(aux1==1){
            for (i = 0; i < max - 1; i++) {
                for (j = i + 1; j < max; j++) {
